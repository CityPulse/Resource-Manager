...
Holds a collection of SAO Ontology objects
...
class SaoInfoException(Exception):
	def __init__(self, message) :
		self.message = message
	def __str__(self) :
		return self.message

class SaoInfo(object):
	...
	def add(self, obj, URI=None):
		if URI == None:
			...
			if hasattr(obj, "URI") == False or obj.URI == None or isBlind(obj):
				#raise SaoInfoException("Tried to add object "+str(obj)+" with no URI !")
		...
		if not self.haveURI(URI):
			...
			if not hasattr(obj, "shortname"):
				raise SaoInfoException("No shortname property for object " + str(obj) + ", did it come from the SAO model ?")
			...
			if isinstance(obj, type(existing)):
				...
				raise SaoInfoException("Tried to add two objects for the same URI and classes are a mismatch !"\
	...
	def findExistingBlindObj(self, o):
		if not hasattr(o, "shortname"):
			raise SaoInfoException("No shortname property for object " + str(o) + ", did it come from the SAO model ?")
